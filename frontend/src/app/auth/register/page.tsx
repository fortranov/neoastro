...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!publicSettings?.email_confirmation_enabled) {
  ...
  return (
    <div className="min-h-screen bg-[#0f0a1e] stars-bg flex items-center justify-center px-4 py-8">
        ...
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 shadow-2xl">
          {success ? (
            <div className="text-center py-8">
              ...
              {publicSettings?.email_confirmation_enabled ? (
                <p className="text-yellow-400 text-sm mt-4">
                  Проверьте вашу почту и подтвердите email для входа.
                </p>
              ) : (
              ...
              <Link
                href="/auth/login"
                className="mt-6 inline-block px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium transition-colors"
              >
                Войти
              </Link>
            </div>
